...
from patchfeld.layout.splitter import Splitter
...
def _build(node, registry, path: tuple[int, ...] = ()) -> Widget:
    if isinstance(node, Panel):
        ...
        widget._patchfeld_spec_path = path  # type: ignore[attr-defined]
        return widget
    if isinstance(node, Tabs):
        ...
        tc._patchfeld_spec_path = path  # type: ignore[attr-defined]
        return tc
    # Container
    ...
    box._patchfeld_spec_path = path  # type: ignore[attr-defined]
    return box
