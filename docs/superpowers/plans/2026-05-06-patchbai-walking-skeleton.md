...
patchbai/
  __init__.py
  __main__.py                  # entry point: python -m patchbai
...
- Create: `.gitignore`
- Create: `patchbai/__init__.py`
- Create: `patchbai/__main__.py`
...
Expected: `Initialized empty Git repository in /Users/jimmy.mills/Developer/patchbai/.git/`
...
[project.scripts]
patchbai = "patchbai.__main__:main"
mt = "patchbai.__main__:main"
...
- [ ] **Step 4: Create `patchbai/__init__.py`**

```python
__version__ = "0.0.1"
```

- [ ] **Step 5: Create `patchbai/__main__.py`** (placeholder; real App wired in Task 15)

```python
def main() -> int:
    print("patchbai scaffolding ready (no app yet)")
    return 0
...
```bash
.venv/bin/python -m patchbai
```

Expected: `patchbai scaffolding ready (no app yet)`
...
```bash
git add pyproject.toml .gitignore patchbai tests
...
Expected: ImportError on `patchbai.events`.

- [ ] **Step 3: Implement `patchbai/events.py`**
...
**Files:**
- Create: `patchbai/layout/__init__.py`
- Create: `patchbai/layout/spec.py`
- Test: `tests/test_layout_spec.py`

- [ ] **Step 1: Create empty `patchbai/layout/__init__.py`**
...
from patchbai.layout.spec import Container, LayoutSpec, Panel
...
Expected: ImportError on `patchbai.layout.spec`.

- [ ] **Step 4: Implement `patchbai/layout/spec.py`**
...
```bash
git add patchbai/layout tests/test_layout_spec.py
...
**Files:**
- Create: `patchbai/persistence/__init__.py`
- Create: `patchbai/persistence/atomic.py`
- Test: `tests/test_atomic.py`

- [ ] **Step 1: Create empty `patchbai/persistence/__init__.py`**
...
Expected: ImportError on `patchbai.persistence.atomic`.

- [ ] **Step 4: Implement `patchbai/persistence/atomic.py`**
...
```bash
git add patchbai/persistence/__init__.py patchbai/persistence/atomic.py tests/test_atomic.py
...
**Files:**
- Create: `patchbai/persistence/paths.py`
...
- [ ] **Step 3: Implement `patchbai/persistence/paths.py`**
...
def global_config_dir() -> Path:
    ...
    return Path(os.environ["HOME"]) / ".config" / "patchbai"
```
...
**Files:**
- Create: `patchbai/persistence/layout_store.py`
...
- [ ] **Step 3: Implement `patchbai/persistence/layout_store.py`**
...
from patchbai.layout.spec import LayoutSpec
from patchbai.persistence.atomic import write_json_atomic
from patchbai.persistence.paths import project_layout_path
...
```bash
git add patchbai/persistence/layout_store.py tests/test_layout_store.py
...
**Files:**
- Create: `patchbai/layout/registry.py`
...
- [ ] **Step 3: Implement `patchbai/layout/registry.py`**
...
```bash
git add patchbai/layout/registry.py tests/test_registry.py
...
**Files:**
- Create: `patchbai/layout/defaults.py`
...
- [ ] **Step 3: Implement `patchbai/layout/defaults.py`**
...
```bash
git add patchbai/layout/defaults.py tests/test_defaults.py
...
**Files:**
- Create: `patchbai/widgets/__init__.py`
- Create: `patchbai/widgets/placeholders.py`

- [ ] **Step 1: Create empty `patchbai/widgets/__init__.py`**

```python
```

- [ ] **Step 2: Implement `patchbai/widgets/placeholders.py`**
...
```bash
git add patchbai/widgets/__init__.py patchbai/widgets/placeholders.py
...
**Files:**
- Create: `patchbai/widgets/orchestrator_chat.py`

- [ ] **Step 1: Implement `patchbai/widgets/orchestrator_chat.py`**
...
```bash
git add patchbai/widgets/orchestrator_chat.py
...
**Files:**
- Create: `patchbai/widgets/chrome.py` (CommandBar; StatusBar appended in Task 12)

- [ ] **Step 1: Implement initial `patchbai/widgets/chrome.py`**
...
```bash
git add patchbai/widgets/chrome.py
...
**Files:**
- Modify: `patchbai/widgets/chrome.py` (append StatusBar)

- [ ] **Step 1: Append `StatusBar` to `patchbai/widgets/chrome.py`**
...
class StatusBar(Horizontal):
    ...
    def on_mount(self) -> None:
        from patchbai.events import StatsUpdated
        bus = self._bus or getattr(self.app, "event_bus", None)
        if bus is None:
            return
...
**Files:**
- Create: `patchbai/layout/engine.py` (diff only; apply added in Task 14)
...
- [ ] **Step 3: Implement diff in `patchbai/layout/engine.py`**
...
**Files:**
- Modify: `patchbai/layout/engine.py` (append `apply`)

- [ ] **Step 1: Append `apply` to `patchbai/layout/engine.py`**
...
```bash
git add patchbai/layout/engine.py
...
from patchbai.widgets.placeholders import ActivityFeed, AgentTable
...
- [ ] **Step 2: Update `patchbai/__main__.py`**
...
```bash
git add patchbai/app.py patchbai/__main__.py tests/test_app_smoke.py
...
async def test_layout_persists_across_app_runs(tmp_path: Path):
    ...
    app1 = PatchbaiApp(cwd=tmp_path)
    async with app1.run_test() as pilot:
        await pilot.pause()
        assert (tmp_path / ".patchbai" / "layout.json").exists()

    # Second run in same cwd: should load the saved layout.
    app2 = PatchbaiApp(cwd=tmp_path)
...
**Files:**
- Create: `patchbai/persistence/transcript_store.py`
...
- [ ] **Step 3: Implement `patchbai/persistence/transcript_store.py`**
...
**Files:**
- Create: `patchbai/orchestrator/__init__.py`
- Create: `patchbai/orchestrator/fake_session.py`
- Test: `tests/test_fake_orchestrator.py`

- [ ] **Step 1: Create empty `patchbai/orchestrator/__init__.py`**
...
from patchbai.events import EventBus, OrchestratorReply, UserMessageToOrchestrator
from patchbai.orchestrator.fake_session import FakeOrchestratorSession
from patchbai.persistence.transcript_store import (
    OrchestratorTranscript,
    TranscriptEntry,
)
...
- [ ] **Step 4: Implement `patchbai/orchestrator/fake_session.py`**
...
```bash
git add patchbai/orchestrator/__init__.py patchbai/orchestrator/fake_session.py tests/test_fake_orchestrator.py
...
- Modify: `patchbai/widgets/orchestrator_chat.py` (already supports `history=`; verify use)
- Modify: `tests/test_app_smoke.py` (append integration test)

- [ ] **Step 1: Modify `patchbai/app.py`** — add fake session + transcript wiring

Replace the existing `PatchbaiApp` class with:
...
The cleanest path is for the OrchestratorChat widget to pull history off the App at mount time. Update its `on_mount` in `patchbai/widgets/orchestrator_chat.py`:
...
```bash
git add patchbai/app.py patchbai/widgets/orchestrator_chat.py tests/test_app_smoke.py
...
```bash
ls -la .patchbai .patchbai/transcripts
cat .patchbai/layout.json
cat .patchbai/transcripts/orchestrator.jsonl
```
...
If the manual run created or modified anything (e.g., `.patchbai/` content) you do NOT want committed, the `.gitignore` already excludes `.patchbai/`. Verify with `git status` that nothing under `.patchbai/` appears.
