# Patchfeld Plan 5 — Rich Widget Library
...
**Architecture:** Most widgets thinly wrap Textual built-ins (`Markdown`, `DirectoryTree`, `TextArea`) with our `Container` shell + props normalization. Three widgets are net-new: `DiffViewer` (renders a unified diff via `rich.syntax`), `LogTail` (polls a file with a 250ms reactive timer), and `Notebook` (`TextArea` over a persisted file in `<cwd>/.patchfeld/scratch/<name>.md`). `Terminal` depends on `textual-terminal` if it installs cleanly against Textual 8.2.5; if not, the task is documented as BLOCKED and the rest of the plan ships without it. `WidgetRegistry.register` grows an optional `description` and `props_schema` so `list_widgets` can return enough for the orchestrator AI to pick widgets and pass props correctly.
...
patchfeld/
  ...
  widgets/
    ...
    notebook.py                 (NEW: TextArea persisted to .patchfeld/scratch/<name>.md)
...
- [ ] **Step 3: Modify `patchfeld/layout/engine.py`**
...
```bash
git add patchfeld/layout/engine.py tests/test_layout_engine_weakref.py
...
- [ ] **Step 3: Modify `patchfeld/config.py`**
...
```bash
git add patchfeld/config.py tests/test_config_general.py
...
- [ ] **Step 3: Modify `patchfeld/layout/registry.py`**
...
- [ ] **Step 4: Modify `patchfeld/orchestrator/tools.py`**
...
In `patchfeld/orchestrator/session.py`, extend `__init__` to accept `widget_registry=None`, store it, and forward in `start`:
...
In `patchfeld/app.py`, find the `OrchestratorSession(...)` construction in `__init__` and add `widget_registry=self.registry`:
...
```bash
git add patchfeld/layout/registry.py patchfeld/orchestrator/tools.py patchfeld/orchestrator/session.py patchfeld/app.py tests/test_widget_registry_metadata.py tests/test_orchestrator_tools_list_widgets.py
...
**Files:**
- Create: `patchfeld/widgets/markdown.py`
...
- [ ] **Step 3: Implement `patchfeld/widgets/markdown.py`**
...
(Add the import at the top of `app.py`: `from patchfeld.widgets.markdown import Markdown`.)
...
```bash
git add patchfeld/widgets/markdown.py patchfeld/app.py tests/test_widget_markdown.py
...
**Files:**
- Create: `patchfeld/widgets/file_viewer.py`
...
- [ ] **Step 3: Implement `patchfeld/widgets/file_viewer.py`**
...
Add import: `from patchfeld.widgets.file_viewer import FileViewer`.
...
```bash
git add patchfeld/widgets/file_viewer.py patchfeld/app.py tests/test_widget_file_viewer.py
...
**Files:**
- Create: `patchfeld/widgets/file_tree.py`
...
- [ ] **Step 3: Implement `patchfeld/widgets/file_tree.py`**
...
Add import: `from patchfeld.widgets.file_tree import FileTree`.
...
```bash
git add patchfeld/widgets/file_tree.py patchfeld/app.py tests/test_widget_file_tree.py
...
**Files:**
- Create: `patchfeld/widgets/diff_viewer.py`
...
- [ ] **Step 3: Implement `patchfeld/widgets/diff_viewer.py`**
...
Add import: `from patchfeld.widgets.diff_viewer import DiffViewer`.
...
```bash
git add patchfeld/widgets/diff_viewer.py patchfeld/app.py tests/test_widget_diff_viewer.py
...
**Files:**
- Create: `patchfeld/widgets/log_tail.py`
...
- [ ] **Step 3: Implement `patchfeld/widgets/log_tail.py`**
...
```bash
git add patchfeld/widgets/log_tail.py patchfeld/app.py tests/test_widget_log_tail.py
...
**Files:**
- Create: `patchfeld/widgets/notebook.py`
- Test: `tests/test_widget_notebook.py`

Persistent scratch space: editable `TextArea` whose contents are loaded from `<cwd>/.patchfeld/scratch/<name>.md` on mount and saved back on every change.
...
- [ ] **Step 3: Implement `patchfeld/widgets/notebook.py`**
...
```bash
git add patchfeld/widgets/notebook.py patchfeld/app.py tests/test_widget_notebook.py
git commit -m "feat(widgets): Notebook — persistent scratch buffer in .patchfeld/scratch/"
```
...
Expected: ImportError on `patchfeld.widgets.terminal` (or skip if textual-terminal not installed).
...
class Terminal(_PtyTerminal):
    ...
    when you want a real `claude` CLI session inside patchfeld rather than the
...
Add import: `from patchfeld.widgets.terminal import Terminal`. Wrap in a `try/except ImportError` if you want graceful degradation when `textual-terminal` is missing — but if Task 11 succeeded, the import should always work.
...
```bash
git add patchfeld/widgets/terminal.py patchfeld/app.py tests/test_widget_terminal.py
