...
patchbai/
  ...
  persistence/
    layouts_store.py            (NEW: NamedLayoutsStore — read/write/list at ~/.config/patchbai/layouts/)
  widgets/
...
```bash
cd /Users/jimmy.mills/Developer/patchbai && .venv/bin/pytest --fixtures 2>&1 | grep -A 2 ok_script
```
...
- [ ] **Step 3: Add `queue_send` to `patchbai/agents/session.py`**
...
In `patchbai/agents/manager.py`, replace `_on_direct_message` with:
...
```bash
git add patchbai/agents/session.py patchbai/agents/manager.py tests/test_agent_session_queue_send.py
...
- [ ] **Step 3: Modify `_on_user_message` in `patchbai/orchestrator/session.py`**
...
```bash
git add patchbai/orchestrator/session.py tests/test_orchestrator_session_prune.py
...
In `patchbai/orchestrator/tools.py`, find the `_ToolSpec(name="spawn_agent", ...)` entry and replace its `input_schema` and `description`:
...
- [ ] **Step 3: Modify `apply` in `patchbai/layout/engine.py`**
...
```bash
git add patchbai/layout/engine.py tests/test_layout_engine_focus.py
...
- [ ] **Step 1: Append two events to `patchbai/events.py`**
...
(Using TYPE_CHECKING avoids a circular import: `patchbai.layout.spec` doesn't import events, so this is safe.)
...
- [ ] **Step 4: Modify `patchbai/layout/engine.py`**
...
```bash
git add patchbai/events.py patchbai/layout/engine.py tests/test_layout_engine_idempotent.py
...
**Files:**
- Create: `patchbai/persistence/layouts_store.py`
- Test: `tests/test_layouts_store.py`

Named layouts live in `~/.config/patchbai/layouts/<name>.json`. The `NamedLayoutsStore` reads/writes them, lists them, and validates them via `LayoutSpec`.
...
- [ ] **Step 3: Implement `patchbai/persistence/layouts_store.py`**
...
```bash
git add patchbai/persistence/layouts_store.py tests/test_layouts_store.py
    ...
    *,
    ...
    if apply_layout is not None and layouts_store is not None:
        layout_specs = [
            ...
            ("save_layout",
             "Save the given LayoutSpec under a name in ~/.config/patchbai/layouts/.",
             {"name": str, "spec": dict},
...
```bash
git add patchbai/orchestrator/tools.py tests/test_orchestrator_tools_layout.py
...
**Files:**
- Create: `patchbai/widgets/layout_switcher.py`
...
- [ ] **Step 3: Implement `patchbai/widgets/layout_switcher.py`**
...
```bash
git add patchbai/widgets/layout_switcher.py tests/test_layout_switcher.py
...
**Files:**
- Create: `patchbai/config.py`
- Test: `tests/test_config_store.py`
- Modify: `pyproject.toml` (add `tomli-w` dep)

`ConfigStore` reads/writes `~/.config/patchbai/config.toml` with a typed schema:
...
```bash
cd /Users/jimmy.mills/Developer/patchbai && uv pip install -e ".[dev]"
```
...
- [ ] **Step 4: Implement `patchbai/config.py`**
...
```bash
git add pyproject.toml patchbai/config.py tests/test_config_store.py
...
- [ ] **Step 3: Implement `patchbai/actions.py`**
...
```bash
git add patchbai/actions.py tests/test_action_registry.py
...
```bash
git add patchbai/orchestrator/tools.py tests/test_orchestrator_tools_config.py
...
**Files:**
- Create: `patchbai/widgets/history_screen.py`
...
- [ ] **Step 3: Implement `patchbai/widgets/history_screen.py`**
...
```bash
git add patchbai/widgets/history_screen.py tests/test_history_screen.py
...
- Modify: `patchbai/widgets/chrome.py` (subscribe StatusBar to LayoutApplied)
...
- [ ] **Step 1: Modify `patchbai/widgets/chrome.py`** — add a LayoutApplied subscription to StatusBar

In `StatusBar.on_mount`, alongside the existing `StatsUpdated` subscription, add:

```python
        from patchbai.events import LayoutApplied
...
- [ ] **Step 2: Replace `patchbai/app.py`**
...
In `patchbai/orchestrator/session.py`, replace `__init__` to add the kwargs:
...
```bash
git add patchbai/app.py patchbai/orchestrator/session.py patchbai/widgets/chrome.py
