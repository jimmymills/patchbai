# Patchfeld Plan 2 — Live Orchestrator + First Child Agent
...
**Files:**
- Create: `patchfeld/agents/__init__.py`
- Create: `patchfeld/agents/state.py`
- Test: `tests/test_agent_state.py`

- [ ] **Step 1: Create empty `patchfeld/agents/__init__.py`**
...
Expected: ImportError on `patchfeld.agents.state`.

- [ ] **Step 4: Implement `patchfeld/agents/state.py`**
...
```bash
git add patchfeld/agents/__init__.py patchfeld/agents/state.py tests/test_agent_state.py
...
Add these dataclasses to `patchfeld/events.py`, keeping the existing types intact:
...
The `from patchfeld.agents.state import AgentInfo` import goes near the top of `events.py` alongside the other imports. Add `from patchfeld.agents.state import AgentState` if you reference it inline; otherwise the string forward ref `"AgentState"` is fine (it's only used in a type annotation).
...
def test_per_agent_transcript_writes_to_agent_id_path(tmp_path: Path):
    ...
    expected = tmp_path / ".patchfeld" / "transcripts" / "abc123.jsonl"
...
- [ ] **Step 3: Modify `patchfeld/persistence/transcript_store.py`**

Replace the contents of `patchfeld/persistence/transcript_store.py` with:
...
Notice the change: `OrchestratorTranscript` now subclasses `AgentTranscript` and passes `agent_id="orchestrator"`. The on-disk path is now `<cwd>/.patchfeld/transcripts/orchestrator.jsonl` — the same file plan 1 wrote — because `project_transcript_path(cwd, "orchestrator")` produces exactly that path.
...
**Files:**
- Create: `patchfeld/persistence/agents_index.py`
- Test: `tests/test_agents_index.py`

The index is a single JSON file `<cwd>/.patchfeld/agents.json` containing an array of `AgentInfo` dicts. Atomic writes via `write_json_atomic`.
...
- [ ] **Step 3: Implement `patchfeld/persistence/agents_index.py`**
...
```bash
git add patchfeld/persistence/agents_index.py tests/test_agents_index.py
...
**Files:**
- Create: `patchfeld/agents/sdk_adapter.py`
...
- [ ] **Step 1: Implement `patchfeld/agents/sdk_adapter.py`**
...
```bash
git add patchfeld/agents/sdk_adapter.py
...
**Files:**
- Create: `patchfeld/agents/fake_sdk_adapter.py`
...
Expected: ImportError on `patchfeld.agents.fake_sdk_adapter`.

- [ ] **Step 3: Implement `patchfeld/agents/fake_sdk_adapter.py`**
...
```bash
git add patchfeld/agents/fake_sdk_adapter.py tests/test_fake_sdk_adapter.py
...
**Files:**
- Create: `patchfeld/agents/session.py`
...
Expected: ImportError on `patchfeld.agents.session`.

- [ ] **Step 3: Implement `patchfeld/agents/session.py`**
...
**Files:**
- Create: `patchfeld/agents/manager.py`
...
from patchfeld.events import AgentSpawned, EventBus
...
Expected: ImportError on `patchfeld.agents.manager`.

- [ ] **Step 3: Implement `patchfeld/agents/manager.py`**
...
**Files:**
- Create: `patchfeld/orchestrator/formatting.py`
...
from patchfeld.orchestrator.formatting import format_assistant_message
...
Expected: ImportError on `patchfeld.orchestrator.formatting`.

- [ ] **Step 3: Implement `patchfeld/orchestrator/formatting.py`**
...
```bash
git add patchfeld/orchestrator/formatting.py tests/test_orchestrator_formatting.py
...
**Files:**
- Create: `patchfeld/orchestrator/tools.py`
...
Expected: ImportError on `patchfeld.orchestrator.tools`.

- [ ] **Step 3: Implement `patchfeld/orchestrator/tools.py`**
...
```bash
git add patchfeld/orchestrator/tools.py tests/test_orchestrator_tools.py
...
**Files:**
- Create: `patchfeld/orchestrator/session.py`
- Delete: `patchfeld/orchestrator/fake_session.py`
...
- [ ] **Step 3: Implement `patchfeld/orchestrator/session.py`**
...
```bash
rm patchfeld/orchestrator/fake_session.py
...
```bash
git add patchfeld/orchestrator/session.py patchfeld/orchestrator/fake_session.py tests/test_fake_orchestrator.py tests/test_orchestrator_session.py
...
**Files:**
- Modify: `patchfeld/widgets/placeholders.py` (drop `AgentTable`; keep `ActivityFeed`)
- Create: `patchfeld/widgets/agent_table.py`
...
from patchfeld.agents.state import AgentInfo, AgentState
from patchfeld.events import AgentSpawned, AgentStateChanged, EventBus
...
Expected: ImportError on `patchfeld.widgets.agent_table`.

- [ ] **Step 3: Strip `AgentTable` from `placeholders.py`**

Open `patchfeld/widgets/placeholders.py` and replace the `AgentTable` class with a deprecated re-export so any stragglers fail loudly:
...
(Just delete the `AgentTable` class — anyone importing `from patchfeld.widgets.placeholders import AgentTable` will now get an ImportError, which is what we want, and `app.py` will be updated in Task 15.)

- [ ] **Step 4: Implement `patchfeld/widgets/agent_table.py`**
...
```bash
git add patchfeld/widgets/placeholders.py patchfeld/widgets/agent_table.py tests/test_agent_table_widget.py
...
**Files:**
- Create: `patchfeld/widgets/agent_transcript.py`
- Create: `patchfeld/widgets/transcript_screen.py`
...
- [ ] **Step 3: Implement `patchfeld/widgets/agent_transcript.py`**
...
- [ ] **Step 4: Implement `patchfeld/widgets/transcript_screen.py`**
...
```bash
git add patchfeld/widgets/agent_transcript.py patchfeld/widgets/transcript_screen.py tests/test_agent_transcript_widget.py
...
- [ ] **Step 1: Replace `patchfeld/app.py` with the plan-2 version**

Open `patchfeld/app.py` and replace its contents with:
...
```bash
cd /Users/jimmy.mills/Developer/patchfeld && .venv/bin/pytest -v
```
...
```bash
git add patchfeld/app.py tests/test_app_smoke.py
...
```bash
git add docs/superpowers/plans/2026-05-06-patchfeld-plan2-live-orchestrator.md
...
- **Carried-forward gotchas:**
  ...
  - `OrchestratorTranscript` is preserved as an alias of `AgentTranscript(agent_id="orchestrator")` so plan-1 transcripts in `<cwd>/.patchfeld/transcripts/orchestrator.jsonl` stay readable.
