...
`patchbai` is a Textual-based terminal application for managing multiple Claude
...
Suggested addition to project `.gitignore`: `.patchbai/`.

### Global: `~/.config/patchbai/`
...
- Launch reads `<cwd>/.patchbai/layout.json`; falls back to built-in dashboard.
...
Single package `patchbai`, entry point `patchbai` (and short `mt`).
